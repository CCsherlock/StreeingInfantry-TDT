 ...
 */
float Motor::ctrlSpeed(float speed, int8_t planIndex, u8 sendFlag,float currentForward)
{
	/*电流输出*/
	//DJI电机丢失发0
	...
	return ctrlCurrent(pidInner.result+currentForward, sendFlag);
}
 ...
 */
float Motor::ctrlPosition(double position, int8_t planIndex, u8 sendFlag,float spdForward,float currentForward)
{
	/*内环*/
	...
	{
		pidOuter.Clear();
		return ctrlSpeed(0, 0, sendFlag,0);
	}
	...
	return ctrlSpeed(pidOuter.result+spdForward, planIndex, sendFlag, currentForward);
}
