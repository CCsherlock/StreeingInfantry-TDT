...
void Dbus_Task(void *pvParameters)
{
	...
	while (1)
	{
		...
		dbusOnlineFlag = xTaskNotifyWait(0xffffffff, 0xffffffff, (uint32_t *)(&SBUS_buf_task), pdMS_TO_TICKS(100));
		if(SBUS_buf_task == 0 && dbusOnlineFlag == 1)
		{
			//清除旧数据
		...
		extern TaskHandle_t StateTask_Handler;
		if(StateTask_Handler)
		{
			BaseType_t xTaskWokenByReceive_Notice; //发送通知用的
			xTaskNotify(StateTask_Handler, (uint32_t)(1), eSetValueWithOverwrite);
		}
	}
}
...
static void dbusHandleData(uint8_t *SBUS_buf)
{
	...
	//右摇杆横向  范围+-660
	RC.Key.CH[0] = my_deathzoom(((SBUS_buf[0] | (SBUS_buf[1] << 8)) & 0x07ff)-1024,5); //!< Channel 0
//	RC.Key.CH[0] = my_deathzoom(RC.Key.CH[0] - 1024, 5);
	//右摇杆纵向   范围+-660
	RC.Key.CH[1] = my_deathzoom((((SBUS_buf[1] >> 3) | (SBUS_buf[2] << 5)) & 0x07ff) - 1024, 5); //!< Channel 1
//	RC.Key.CH[1] = my_deathzoom(RC.Key.CH[1] - 1024, 5);
	//左摇杆横向   范围+-660
	RC.Key.CH[2] = my_deathzoom((((SBUS_buf[2] >> 6) | (SBUS_buf[3] << 2) | (SBUS_buf[4] << 10)) & 0x07ff) - 1024, 5); //!< Channel 2
//	RC.Key.CH[2] = my_deathzoom(RC.Key.CH[2] - 1024, 5);
	//左摇杆纵向   范围+-660
	RC.Key.CH[3] = my_deathzoom((((SBUS_buf[4] >> 1) | (SBUS_buf[5] << 7)) & 0x07ff) - 1024, 5); //!< Channel 3
